...
struct element_getter {
  ...
  operator()(matrix_type<ROWS, COLS> &m, unsigned int row, unsigned int col) const {
  ...
  template<unsigned int ROWS, unsigned int COLS>
  ALGEBRA_HOST_DEVICE inline scalar
  operator()(const matrix_type<ROWS, COLS> &m, unsigned int row, unsigned int col) const {
...
using transform3 = cmath::transform3<storage_type, scalar, ROOT::Math::SMatrix<scalar, 4, 4>, element_getter, block_getter >;
...
auto theta = [](const auto& a) { return cmath::theta<smatrix::storage_type>(a); };
...
auto cross = [](const auto& a, const auto& b) { return cmath::cross<smatrix::storage_type>(a, b); };
auto dot = [](const auto& a, const auto& b) { return cmath::dot<smatrix::storage_type>(a, b); };
auto normalize = [](const auto& a) { return cmath::normalize<smatrix::storage_type>(a); };
