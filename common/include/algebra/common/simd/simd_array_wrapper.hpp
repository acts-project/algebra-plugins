...
struct array4_wrapper {
   ...
   *
   */
  array4_wrapper(scalar_t v1, scalar_t v2, scalar_t v3,
   ...
   */
  inline array4_wrapper<scalar_t> &operator=(
      const array4_wrapper<scalar_t> &other) {
    _array = other._array;
    return *this;
  }
   ...
   */
  inline array4_wrapper<scalar_t> &operator=(
      std::initializer_list<scalar_t> &list) {
  ...
  inline decltype(auto) operator[](const unsigned int i) const {
    return _array[i];
  }
   ...
   */
  inline decltype(auto) operator*=(scalar_t factor) { return _array *= factor; }
};

/** Operator overload from SimdArray for simd array wrapper that handles the
 * default floating point literal conversion.
 *
 ...
 */
template <typename scalar_t>
inline decltype(auto) operator+(const array4_wrapper<scalar_t> &lhs,
                                const double &rhs) {
 ...
 */
template <typename scalar_t>
inline decltype(auto) operator+(const double &lhs,
                                const array4_wrapper<scalar_t> &rhs) {
 ...
 */
template <typename scalar_t, typename other_type>
inline decltype(auto) operator+(const other_type &lhs,
                                const array4_wrapper<scalar_t> &rhs) {
...
/** Equality operator definition for simd array wrapper in a simd Vector4
 * struct.
 *
 * @tparam scalar_t value type used in the wrapper
 *
...
inline decltype(auto) operator==(
    const Vector4<simd::array4_wrapper<scalar_t>> &lhs,
