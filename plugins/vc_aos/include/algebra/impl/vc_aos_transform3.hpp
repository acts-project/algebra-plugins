...
struct transform3 {
    ...
    transform3(const matrix44 &m, const matrix44 &m_inv)
        ...
        if constexpr (std::floating_point<scalar_type>) {
            ...
            assert(
                algebra::approx_equal(invert(m), m_inv, 16.f * epsilon, 1e-6f));
        }
    }
    ...
    constexpr matrix44 invert(const matrix44 &m) const {
        ...
        // i[e_z][3] = 0;
        i[e_t][0] = m[e_t][0] * m[e_z][1] * m[e_y][2] -
                    m[e_z][0] * m[e_t][1] * m[e_y][2] -
                    m[e_t][0] * m[e_y][1] * m[e_z][2] +
                    m[e_y][0] * m[e_t][1] * m[e_z][2] +
                    m[e_z][0] * m[e_y][1] * m[e_t][2] -
                    m[e_y][0] * m[e_z][1] * m[e_t][2];
        i[e_t][1] = m[e_z][0] * m[e_t][1] * m[e_x][2] -
                    m[e_t][0] * m[e_z][1] * m[e_x][2] +
                    m[e_t][0] * m[e_x][1] * m[e_z][2] -
                    m[e_x][0] * m[e_t][1] * m[e_z][2] -
                    m[e_z][0] * m[e_x][1] * m[e_t][2] +
                    m[e_x][0] * m[e_z][1] * m[e_t][2];
        i[e_t][2] = m[e_t][0] * m[e_y][1] * m[e_x][2] -
                    m[e_y][0] * m[e_t][1] * m[e_x][2] -
                    m[e_t][0] * m[e_x][1] * m[e_y][2] +
                    m[e_x][0] * m[e_t][1] * m[e_y][2] +
                    m[e_y][0] * m[e_x][1] * m[e_t][2] -
                    m[e_x][0] * m[e_y][1] * m[e_t][2];
        // i[e_t][3] = 1;
