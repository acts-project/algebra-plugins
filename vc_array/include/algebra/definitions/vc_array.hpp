...
ALGEBRA_HOST_DEVICE
inline std::array<scalar, 2> operator*(const std::array<scalar, 2> &a,
                                       const scalar s) {
  return {a[0] * s, a[1] * s};
}

ALGEBRA_HOST_DEVICE
inline std::array<scalar, 2> operator*(const scalar s,
...
ALGEBRA_HOST_DEVICE inline scalar dot(const vector_type &a,
                                      const vector_type &b) {
  return (a * b).sum();
}
...
ALGEBRA_HOST_DEVICE inline scalar dot(const vec_expr1 &a, const vec_expr2 &b) {
  return (a * b).sum();
}
 ...
 **/
ALGEBRA_HOST_DEVICE
inline scalar dot(const std::array<scalar, 2> &a,
                  const std::array<scalar, 2> &b) {
  return (a[0] * b[0] + a[1] * b[1]);
}
 ...
 **/
template <typename vector_type>
ALGEBRA_HOST_DEVICE inline vector_type cross(const vector_type &a,
                                             const vector_type &b) {
 ...
 **/
template <typename vec_expr1, typename vec_expr2>
ALGEBRA_HOST_DEVICE inline auto cross(const vec_expr1 &a, const vec_expr2 &b)
    -> decltype(a * b - a * b) {
  return {a[1] * b[2] - b[1] * a[2], a[2] * b[0] - b[2] * a[0],
          a[0] * b[1] - b[0] * a[1], 0};
}
...
ALGEBRA_HOST_DEVICE inline simd::array<scalar, 4> vector(
...
ALGEBRA_HOST_DEVICE inline auto block(const matrix_type &m, unsigned int row,
  ...
  simd::aligned::vector<decltype(m.x)> vecs = {std::move(m.x), std::move(m.y),
...
struct transform3 {
  ...
  ALGEBRA_HOST_DEVICE
  inline auto rotation() const { return getter::block<3, 3>(_data, 0, 0); }
