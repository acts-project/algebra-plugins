...
ALGEBRA_HOST_DEVICE inline auto phi(const vec_expr_type &v) noexcept {
...
ALGEBRA_HOST_DEVICE inline auto theta(const SVector<scalar, kDIM> &v) noexcept {
 ...
 **/
template <unsigned int kDIM>
ALGEBRA_HOST_DEVICE inline auto norm(const SVector<scalar, kDIM> &v) {
 ...
 **/
template <unsigned int kDIM>
ALGEBRA_HOST_DEVICE inline auto eta(const SVector<scalar, kDIM> &v) noexcept {
 ...
 **/
template <typename vec_expr_type>
ALGEBRA_HOST_DEVICE inline auto perp(const vec_expr_type &v) noexcept {
  // static_assert(kDIM >= 2, "vector::perp() required rows >= 2.");
  scalar element0 = v.apply(0);
  scalar element1 = v.apply(1);
...
struct transform3 {
  ...
  transform3(const vector3 &t, const vector3 &z, const vector3 &x) {
    ...
    // This should be an exception, "transform3 could not be initialized. Matrix
    // not invertible."
    assert(ifail == 0);
  }
  ...
  ALGEBRA_HOST_DEVICE
  inline auto matrix() const { return _data; }
  ...
  inline const point3 point_to_global(const point3 &v) const {
    ...
    return SVector<scalar, 4>(_data * vector_4)
        .template Sub<SVector<scalar, 3>>(0);
  }
  ...
  inline const point3 point_to_local(const point3 &v) const {
    ...
    vector_4.Place_at(v, 0);
    vector_4[3] = static_cast<scalar>(1);
    return SVector<scalar, 4>(_data_inv * vector_4)
        .template Sub<SVector<scalar, 3>>(0);
  }
...
struct cartesian2 {
  ...
  ALGEBRA_HOST_DEVICE inline const auto operator()(const point3_type &v) const {
   ...
   **/
  ALGEBRA_HOST_DEVICE
  inline const auto operator()(const transform3 &trf, const point3 &p) const {
...
ALGEBRA_HOST_DEVICE inline auto dot(const vector3_type &a,
                                    const vecexpr3_type &b) {
 ...
 **/
template <typename vector3_type, typename vecexpr3_type>
ALGEBRA_HOST_DEVICE inline auto cross(const vecexpr3_type &a,
                                      const vector3_type &b) {
