...
class transform_fixture : public device_fixture<A, algebra::get_scalar_t<A>> {
...
class transform3_ops_functor {
    ...
    using scalar_t = algebra::get_scalar_t<A>;
    using point3_t = algebra::get_point3D_t<A>;
    using vector3_t = algebra::get_vector3D_t<A>;
    using transform3_t = algebra::get_transform3D_t<A>;
    ...
    scalar_t transform3_ops(vector3_t t1, vector3_t t2, vector3_t t3,
        ...
        return {algebra::vector::norm(translation) +
                algebra::vector::perp(gpoint) + algebra::vector::phi(lpoint) +
                algebra::vector::dot(gvec, lvec)};
    }
};
