...
struct column_wise_operator {
  ...
  inline matrix_type<3, 3> cross(const matrix_type<3, 3>& m,
    ...
    matrix_actor().set_block(ret, vector_actor().cross(m_col0, v), 0, 0);
    matrix_actor().set_block(ret, vector_actor().cross(m_col1, v), 0, 1);
    matrix_actor().set_block(ret, vector_actor().cross(m_col2, v), 0, 2);
