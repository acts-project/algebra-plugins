...
class matrix_fixture : public device_fixture<A, algebra::get_scalar_t<A>> {

  using index_t = algebra::get_size_t<A>;
  using scalar_t = algebra::get_scalar_t<A>;
  template <index_t R, index_t C>
  using matrix_t = algebra::get_matrix_t<A, R, C>;
...
class matrix22_ops_functor {
 ...
 private:
  ...
  scalar_t matrix22_ops(const matrix_t<2, 2>& m22) const {
    ...
    // Test Zero
    auto m23 = algebra::matrix::zero<matrix_t<2, 3>>();
    ...
    m32 = m32 + algebra::matrix::identity<matrix_t<3, 2>>();
