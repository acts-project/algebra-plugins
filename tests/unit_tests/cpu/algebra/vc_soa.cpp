...
TEST(test_vc_host, vc_soa_transform3) {
    ...
    static_assert(
        std::same_as<decltype(trf1_cast_f), vc_soa::transform3<float>>);
    static_assert(
    ...
    for (int j = 0; j < 3; ++j) {
        for (int i = 0; i < 3; ++i) {
            ...
            EXPECT_TRUE(
                (mat_f[i][j] == algebra::cast_to<float>(elem_ij)).isFull());
            EXPECT_TRUE(
                (mat_d[i][j] == algebra::cast_to<double>(elem_ij)).isFull());
            EXPECT_TRUE(
                (mat_i[i][j] == algebra::cast_to<int>(elem_ij)).isFull());
        }
    }
...
TEST(test_vc_host, vc_soa_matrix3) {
    ...
    static_assert(
        std::is_same_v<algebra::traits::value_t<matrix_2x3_t>, float>);
    ...
    static_assert(
        algebra::traits::is_square<vc_soa::matrix_type<value_t, 2, 2>>);
    static_assert(
        algebra::traits::is_square<vc_soa::matrix_type<value_t, 3, 3>>);
}
...
TEST(test_vc_host, vc_soa_matrix64) {
    ...
    static_assert(
        algebra::traits::is_square<vc_soa::matrix_type<value_t, 4, 4>>);
    static_assert(
        algebra::traits::is_square<vc_soa::matrix_type<value_t, 6, 6>>);
