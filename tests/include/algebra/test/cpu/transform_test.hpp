...
TYPED_TEST_P(transform_test, transform3D) {
    ...
    for (algebra::get_size_t<TypeParam> j = 0; j < 3; ++j) {
        for (algebra::get_size_t<TypeParam> i = 0; i < 2; ++i) {
            ...
            ASSERT_FLOAT_EQ(elem_i, static_cast<float>(
                                        algebra::getter::element(mat_f, i, j)));
            ASSERT_FLOAT_EQ(
                elem_inv_i,
                static_cast<float>(algebra::getter::element(mat_inv_f, i, j)));
        }
    }
    ...
    for (algebra::get_size_t<TypeParam> j = 0; j < 3; ++j) {
        for (algebra::get_size_t<TypeParam> i = 0; i < 2; ++i) {
            ...
            ASSERT_DOUBLE_EQ(
                elem_i,
                static_cast<double>(algebra::getter::element(mat_d, i, j)));
            ASSERT_DOUBLE_EQ(
                elem_inv_i,
                static_cast<double>(algebra::getter::element(mat_inv_d, i, j)));
        }
    }
    ...
    for (algebra::get_size_t<TypeParam> j = 0; j < 3; ++j) {
        for (algebra::get_size_t<TypeParam> i = 0; i < 2; ++i) {
            ...
            ASSERT_EQ(elem_inv_i, static_cast<int>(algebra::getter::element(
                                      mat_inv_i, i, j)));
        }
    }
...
TYPED_TEST_P(transform_test, global_transformations) {
    ...
    algebra::get_vector3D_t<TypeParam> lvectorC =
        trf.vector_to_global(gvectorB);
